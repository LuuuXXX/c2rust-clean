...
`C2RUST_PROJECT_ROOT` 环境变量用于指定项目的根目录路径。该环境变量被 git_helper 模块使用，用于定位 `.c2rust` 目录并管理 Git 版本控制。
...
**注意**：如果使用 git_helper 功能（自动提交 `.c2rust` 目录的更改），必须设置此环境变量。

## Git 版本控制

工具集成了 `git_helper` 模块，用于自动跟踪和提交 `.c2rust` 目录的变更：

- **自动初始化**：首次运行时，会在 `<C2RUST_PROJECT_ROOT>/.c2rust/.git` 自动初始化 Git 仓库
- **变更检测**：每次执行清理命令后，自动检测 `.c2rust` 目录下的修改
- **自动提交**：如果检测到变更，自动执行 git commit 保存修改信息
- **提交信息**：自动生成包含时间戳的提交信息

这确保了配置变更的完整历史记录，便于追溯和回滚。
