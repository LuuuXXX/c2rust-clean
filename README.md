...
`c2rust-clean` 是一个命令行工具，用于在当前目录执行 C 项目清理命令。该工具是 c2rust 工作流的一部分，用于管理从 C 到 Rust 项目的转换过程。工具会自动检测项目根目录（包含 `.c2rust` 目录的位置），并保存当前执行目录相对于项目根目录的路径。
...
4. **相对路径保存** - 清理目录会自动保存为相对于项目根目录的路径
